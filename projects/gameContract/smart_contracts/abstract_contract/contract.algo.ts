 ...
 * Timelines are based on Block Rounds.
 */
...
export class GameContract extends Contract {
   ...
   * Standard flow for single-round games.
   */
  ...
  protected getRequiredMBR(command: 'newGame' | 'join'): uint64 {
    if (command === 'newGame') {
      // Box 'gs': Key (2 prefix + 8 ID = 10) + Value (32 bytes: 4 * uint64)
      const configMBR = this.getBoxMBR(10, 32)
  ...
  /**
   * Allows registration without immediate commit.
   * Useful for multi-round games where commits happen later.
   */
  ...
  /**
   * Commit in a specific round.
   * Note: Timeline validation is delegated to the subclass.
   */
  ...
  /**
   * Reveal for a specific round.
   * Stores the choice and deletes the commit blob to free MBR.
   */
